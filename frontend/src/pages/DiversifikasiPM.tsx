...
const DATE_FIELDS_PM = ["tglPenerimaan", "pmTglAnalisa", "pmTglReport"] as const

const INFO_FIELDS = ["tglPenerimaan", "kodeItem", "namaMaterial", "manufacture", "noBatchMaterial"]

function canEdit(division: string | undefined, field: string): boolean {
  if (division === "Admin") return true
  if (division === "TS")    return true
  if (division === "CPro")  return INFO_FIELDS.includes(field)
  return false
}
const canEditStatusProject = (div?: string) => div === "Admin" || div === "TS"
const canIsiUlang          = (div?: string) => div !== "TS"

function isNoLabscale(kodeItem: string): boolean {
  const trimmed = kodeItem.trim()
  return trimmed.startsWith("2") || trimmed.startsWith("3")
}
...
const EMPTY_PRODUCT: DiversifikasiProdukPM = {
  ...
  stabtestSensoriDFCT: "", stabtestKeterangan: "", stabtestStatus: "",
}
...
function CopyableTooltip({ text, children }: { text: string; children: React.ReactNode }) {
  ...
  return (
    <div ref={triggerRef} className="relative inline-block w-full" onMouseEnter={handleTriggerEnter} onMouseLeave={scheduleHide}>
      {children}
...
const BADGE_MAP_STATUS_PROJECT: Record<string, string> = {
...
function BadgeStatusHasil({ value }: { value: string }) {
  if (!value) return <span className="text-gray-300 dark:text-gray-600 text-xs">—</span>
  const cls = BADGE_MAP_STATUS_HASIL[value] ?? "bg-gray-500 text-white dark:bg-gray-600 dark:text-gray-100"
  return <span className={`inline-block px-2 py-0.5 rounded text-[11px] font-semibold whitespace-nowrap ${cls}`}>{value}</span>
}
function BadgeStatusProject({ value }: { value: string }) {
  if (!value) return <span className="text-gray-300 dark:text-gray-600 text-xs">—</span>
  const cls = BADGE_MAP_STATUS_PROJECT[value] ?? "bg-gray-500 text-white dark:bg-gray-600 dark:text-gray-100"
  return <span className={`inline-block px-2 py-0.5 rounded text-[11px] font-semibold whitespace-nowrap ${cls}`}>{value}</span>
}
...
function FieldWrapper({ field, division, children }: { field: string; division: string | undefined; children: React.ReactNode }) {
...
function FInput({ label, name, value, onChange, type = "text", disabled = false }: {
  label: string; name: string; value: string; onChange: (v: string) => void; type?: string; disabled?: boolean
}) {
  return (
    <div>
...
function FSelect({ label, name, value, onChange, options, disabled = false }: {
  label: string; name: string; value: string; onChange: (v: string) => void; options: readonly string[]; disabled?: boolean
}) {
  return (
    <div>
      <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">{label}</label>
...
function ProductListEditor({ products, onChange, canEditProd, onSaveProduk, refreshKey }: {
  products: DiversifikasiProdukPM[]
  onChange: (p: DiversifikasiProdukPM[]) => void
  canEditProd: boolean
  ...
  useEffect(() => {
    const next = products.map(p => {
      ...
      return {
        ...p,
        evaluasiAsKemasan:      p.evaluasiAsKemasan      || "OP",
  ...
  if (!canEditProd) return (
    <p className="text-xs text-gray-400 italic py-2">Bagian ini hanya bisa diisi oleh Admin atau TS.</p>
  )
        ...
        <div key={i} className="border border-gray-200 dark:border-neutral-700 rounded-xl p-4 space-y-3 bg-gray-50/60 dark:bg-neutral-800/40">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input id={`pm-tglKirimQC-${i}`} name={`pm-tglKirimQC-${i}`} type="date" value={p.produkTglKirimQC ? String(p.produkTglKirimQC).slice(0, 10) : ""}
            ...
            <div>
              ...
              <input id={`pm-tglKeluarHasil-${i}`} name={`pm-tglKeluarHasil-${i}`} type="date" value={p.produkTglKeluarHasil ? String(p.produkTglKeluarHasil).slice(0, 10) : ""}
            ...
            {p.produkTglKirimQC && (
              <div className="col-span-2">
                <LeadTimeBadge
                  tglKirim={p.produkTglKirimQC ? String(p.produkTglKirimQC) : null}
                  tglKeluar={p.produkTglKeluarHasil ? String(p.produkTglKeluarHasil) : null}
                />
              </div>
            )}
            <div className="col-span-2">
              <FSelect label="Evaluasi as Kemasan" name="evaluasiAsKemasan" value={p.evaluasiAsKemasan}
                onChange={v => upd(i, "evaluasiAsKemasan", v)} options={HASIL_OPTIONS} />
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <FSelect label="Status" name="stabtestStatus" value={p.stabtestStatus}
              onChange={v => upd(i, "stabtestStatus", v)} options={STATUS_HASIL_OPTIONS} />
            <div className="col-span-2">
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Keterangan Stabtest</label>
              <textarea id={`pm-stabtestKeterangan-${i}`} name={`pm-stabtestKeterangan-${i}`} value={p.stabtestKeterangan} rows={2} onChange={e => upd(i, "stabtestKeterangan", e.target.value)}
...
function ProductDetailModal({ products, namaMaterial, onClose }: {
  products: DiversifikasiProdukPM[]; namaMaterial: string; onClose: () => void
}) {
  return ReactDOM.createPortal(
    <div style={OV_CENTER} onClick={onClose}>
      <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-neutral-700 w-full max-w-6xl max-h-[85vh] flex flex-col"
        onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-neutral-800">
          <div>
            <h3 className="text-sm font-bold text-gray-800 dark:text-gray-100">Detail Alokasi Produk Labscale &amp; Stabtest</h3>
            <p className="text-xs text-gray-400 mt-0.5">{namaMaterial}</p>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs bg-violet-100 dark:bg-violet-900/50 text-violet-700 dark:text-violet-300 px-2.5 py-1 rounded-full font-semibold">
          ...
          <table className="w-full border-collapse text-xs" style={{ minWidth: "1100px" }}>
            <thead>
              <tr style={{ height: "36px" }}>
                ...
                <th colSpan={10} className="px-3 bg-[#7c3aed] text-white text-[10px] font-bold text-center border-r border-white/20" style={{ verticalAlign: "middle" }}>ALOKASI PRODUK LABSCALE KEMASAN PRIMER</th>
                <th colSpan={6} className="px-3 bg-[#047857] text-white text-[10px] font-bold text-center" style={{ verticalAlign: "middle" }}>STABTEST</th>
              </tr>
              <tr style={{ height: "36px" }}>
                {["Kode Produk", "Lead Time", "Tgl Kirim QC", "Tgl Keluar", "Evaluasi Kemasan", "Fisik", "Kimia", "Mikro", "Sensori", "Cek Karakt."].map(h => (
                ...
                {["Fisik", "Kimia", "Mikro", "Sensori DFCT", "Keterangan", "Status"].map(h => (
            ...
            <tbody>
              {products.map((prod, i) => (
                <tr key={i} className={`border-b border-gray-100 dark:border-neutral-800 transition-colors
                  ${i % 2 === 0 ? "bg-white dark:bg-neutral-900" : "bg-violet-50/20 dark:bg-violet-950/10"}
                  hover:bg-violet-50/60 dark:hover:bg-violet-950/30`}>
                  ...
                  <td className="px-3 py-3 text-center"><BadgeHasil value={prod.evaluasiAsKemasan || ""} /></td>
                  ...
                  <td className="px-3 py-3 text-center"><BadgeHasil value={prod.produkCekKarakteristik || ""} /></td>
                  ...
                  <td className="px-3 py-3 bg-emerald-50/50 dark:bg-emerald-950/20 max-w-[120px]">
                    {prod.stabtestKeterangan
                      ? <CopyableTooltip text={prod.stabtestKeterangan}>
                        <span className="block truncate text-gray-600 dark:text-gray-300">{prod.stabtestKeterangan}</span>
                      </CopyableTooltip>
                      : <D />}
                  </td>
                  <td className="px-3 py-3 bg-emerald-50/50 dark:bg-emerald-950/20 text-center"><BadgeStatusHasil value={prod.stabtestStatus || ""} /></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="px-6 py-3 border-t border-gray-100 dark:border-neutral-800 flex justify-end">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium rounded-lg border border-gray-200 dark:border-neutral-700 text-gray-500 hover:border-gray-400 transition-colors">
            Tutup
          </button>
        </div>
      </div>
...
export default function DiversifikasiPMPage() {
  useTheme()
  ...
  const [pendingTrialProduk, setPendingTrialProduk] = useState<string | null>(null)

  const [expandedRevisions, setExpandedRevisions] = useState<Set<string>>(new Set())
  const [productModal, setProductModal] = useState<{
    open: boolean; products: DiversifikasiProdukPM[]; namaMaterial: string
  ...
  useEffect(() => {
    if (!form.pmTglAnalisa) return
    setForm(p => ({ ...p, pmHasilAnalisa: p.pmHasilAnalisa || "On Progress" }))
  }, [form.pmTglAnalisa])
  ...
  const openEdit = (row: DiversifikasiPM) => {
    ...
    setForm({
      ...
      tglPenerimaan: fmtDateRaw(row.tglPenerimaan),
      kodeItem: row.kodeItem || "",
      namaMaterial: row.namaMaterial || "",
      manufacture: row.manufacture || "",
      noBatchMaterial: row.noBatchMaterial || "",
      pmTglAnalisa: fmtDateRaw(row.pmTglAnalisa),
      pmTglReport: fmtDateRaw(row.pmTglReport),
      pmHasilAnalisa: (row.pmHasilAnalisa || "") as StatusRM | "",
      pmKeterangan: row.pmKeterangan || "",
      trialKodeProduk: row.trialKodeProduk || "",
      trialNoBatch: row.trialNoBatch || "",
      trialHasilFinal: (row.trialHasilFinal || "") as HasilAnalisa | "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const f of DATE_FIELDS_PM) { payload[f] = toDatetime(form[f] as string) }
    payload.products = form.products.map(p => {
      const pp: Record<string, unknown> = { ...p }
      delete pp.id; delete pp.diversifikasiPmId
      for (const f of ["produkTglKirimQC", "produkTglKeluarHasil"] as const) {
        const val = pp[f]
    ...
    try {
      const url    = isEdit && form.id ? `${API_BASE}/diversifikasi-pm/${form.id}` : `${API_BASE}/diversifikasi-pm`
      const method = isEdit ? "PUT" : "POST"
      const res    = await fetch(url, { method, headers: authHeaders, body: JSON.stringify(payload) })
      if (!res.ok) {
        const e = await res.json().catch(() => ({ message: "Gagal" }))
  ...
  const handleDelete = async (id?: number) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/diversifikasi-pm/${id}`, { method: "DELETE", headers: authHeaders })
      if (!res.ok) throw new Error()
  ...
  const saveNewItemToDB = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/master-items-pm`, {
        method: "POST", headers: authHeaders,
        body: JSON.stringify({ kodeItem: form.kodeItem, namaMaterial: form.namaMaterial, manufacture: form.manufacture || "" }),
      })
      if (res.ok) { setPendingNewItem(null); setShowNewItemConfirm(false); alert("Item baru berhasil disimpan ke master PM") }
  ...
  const saveNewProdukToDB = async (kodeProduk: string): Promise<void> => {
    ...
    try {
      ...
      else if (res.status === 409) { setProdukRefreshKey(k => k + 1); alert(`Kode produk "${kodeProduk}" sudah terdaftar di database`) }
      else { const e = await res.json().catch(() => ({})); alert(`Gagal menyimpan kode produk: ${e.error || "Unknown"}`) }
    } catch { alert("Gagal menyimpan kode produk baru") }
  }

  const toggleRevision = (nomorPM: string) => {
    setExpandedRevisions(prev => {
      const next = new Set(prev)
      if (next.has(nomorPM)) next.delete(nomorPM); else next.add(nomorPM)
      return next
    })
  }
  ...
  const renderRow = (row: DiversifikasiPM, idx: number, isRevision = false) => {
    ...
    const bgBase = isRevision
      ...
      : idx % 2 === 0
        ...
        : "bg-slate-50/80 dark:bg-neutral-950"
      ...
      <tr key={`${isRevision ? "rev" : "main"}-${row.id}`}
        ...
        <td className="px-3 py-2.5 whitespace-nowrap">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-mono font-bold text-[#2e3192] dark:text-indigo-400">{row.nomorPM}</span>
            {hasRevisions && (
              <button onClick={() => toggleRevision(row.nomorPM)}
        ...
        <td className="px-3 py-2.5 text-center"><BadgeStatusProject value={row.statusProject || ""} /></td>

        <td className="px-3 py-2.5 whitespace-nowrap"><DateVal v={row.tglPenerimaan} /></td>
        ...
        <td className="px-3 py-2.5" style={{ minWidth: "220px", maxWidth: "320px" }}>
          {row.namaMaterial ? <span className="text-xs text-gray-700 dark:text-gray-200 font-medium">{row.namaMaterial}</span> : <D />}
        </td>
        <td className="px-3 py-2.5" style={{ minWidth: "180px", maxWidth: "260px" }}>
          {row.manufacture ? <span className="text-xs text-gray-700 dark:text-gray-200">{row.manufacture}</span> : <D />}
        </td>
        ...
        <td className="px-3 py-2.5 bg-sky-50/50 dark:bg-sky-950/10 text-center min-w-[90px] whitespace-nowrap">
          <LeadTimeBadge tglKirim={row.pmTglAnalisa} tglKeluar={row.pmTglReport} compact />
        </td>
        <td className="px-3 py-2.5 bg-sky-50/50 dark:bg-sky-950/10 whitespace-nowrap"><DateVal v={row.pmTglAnalisa} /></td>
        <td className="px-3 py-2.5 bg-sky-50/50 dark:bg-sky-950/10 whitespace-nowrap"><DateVal v={row.pmTglReport} /></td>
        <td className="px-3 py-2.5 bg-sky-50/50 dark:bg-sky-950/10 text-center"><BadgeStatusHasil value={row.pmHasilAnalisa || ""} /></td>
        <td className="px-3 py-2.5 bg-sky-50/50 dark:bg-sky-950/10 max-w-[140px]">
          {row.pmKeterangan
            ? <CopyableTooltip text={row.pmKeterangan}><span className="block truncate text-xs text-gray-600 dark:text-gray-300">{row.pmKeterangan}</span></CopyableTooltip>
            : <D />}
        </td>
        ...
        <td className="px-3 py-2.5 bg-emerald-50/50 dark:bg-emerald-950/10 whitespace-nowrap text-[11px] font-semibold text-[#2e3192] dark:text-indigo-400">
          {row.trialKodeProduk || <D />}
        </td>
        <td className="px-3 py-2.5 bg-emerald-50/50 dark:bg-emerald-950/10 whitespace-nowrap"><Val v={row.trialNoBatch} /></td>
        <td className="px-3 py-2.5 bg-emerald-50/50 dark:bg-emerald-950/10 text-center"><BadgeHasil value={row.trialHasilFinal || ""} /></td>
        ...
        <td className="px-3 py-2.5">
          {isRevision ? (
            ...
            <div className="flex items-center gap-1">
              ...
              {row.pmHasilAnalisa === "Reject" && canIsiUlang(division) && (
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-neutral-950 px-8 py-7 font-sans transition-colors duration-300 space-y-4">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">Diversifikasi PM</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Monitoring Diversifikasi Packaging Material</p>
        </div>
            ...
            <table className="w-full border-collapse text-xs" style={{ minWidth: "1600px" }}>
              <thead>
                <tr>
                  ...
                  <th colSpan={5} className="py-2 text-center text-[10px] font-bold text-white bg-[#2e3192] sticky top-0 z-20 shadow border-x border-white/20">INFORMASI UMUM</th>
                  <th colSpan={5} className="py-2 text-center text-[10px] font-bold text-white bg-[#0369a1] sticky top-0 z-20 shadow border-x border-white/20">PACKAGING MATERIAL</th>
                  ...
                  <th colSpan={5} className="py-2 text-center text-[10px] font-bold text-white bg-[#047857] sticky top-0 z-20 shadow border-x border-white/20">TRIAL MESIN</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <React.Fragment key={`main-${row.id}`}>
                    ...
                    {expandedRevisions.has(row.nomorPM) && row.revisions?.map(rev =>
          ...
          <div className="relative bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-700 rounded-2xl w-full max-w-4xl shadow-2xl my-auto">
            ...
            <div className="flex items-center justify-between px-7 py-5 border-b border-gray-100 dark:border-neutral-800">
              <div>
                <h3 className="text-base font-bold text-gray-800 dark:text-gray-100">
                  {isIsiUlang ? `Isi Ulang Data — ${form.nomorPM}` :
                    isEdit ? "Edit Data Diversifikasi PM" : "Tambah Data Diversifikasi PM"}
                </h3>
            ...
            {isIsiUlang && (
              <div className="mx-7 mt-3 px-4 py-2.5 rounded-lg border border-blue-200 bg-blue-50 dark:bg-blue-950/30 dark:border-blue-800 text-xs text-blue-700 dark:text-blue-300 flex items-center gap-2">
                <RefreshIcon />
                <span>Mengisi ulang <strong>{form.nomorPM}</strong>. Data lama tetap tersimpan sebagai riwayat.</span>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit}>
              <div className="px-7 py-6 grid grid-cols-2 gap-x-5 gap-y-3 max-h-[65vh] overflow-y-auto">
                ...
                <FieldWrapper field="tglPenerimaan" division={division}>
                  <FInput label="Tgl Penerimaan" name="tglPenerimaan" value={form.tglPenerimaan}
                    onChange={set("tglPenerimaan")} type="date" disabled={!canEdit(division, "tglPenerimaan")} />
                </FieldWrapper>

                <div className="col-span-2">
                  <ItemAutocomplete
                    kodeItemValue={form.kodeItem}
                    namaMaterialValue={form.namaMaterial}
                    manufactureValue={form.manufacture}
                    ...
                    apiEndpoint={`${API_BASE}/master-items-pm`}
                          ...
                          <div>
                            ...
                            <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
                              {pendingNewItem.isManufactureOnly
                                ...
                                : <><strong>{form.kodeItem}</strong> — {form.namaMaterial}{form.manufacture && ` (${form.manufacture})`} tidak ada di database PM.</>
                              }
                            </p>
                          </div>
                        </div>
                ...
                <SecHeader title="Packaging Material" badges={[
                  { text: "TS", color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" },
                ]} />

                <FieldWrapper field="pmTglAnalisa" division={division}>
                  <FInput label="Tgl Analisa" name="pmTglAnalisa" value={form.pmTglAnalisa}
                    onChange={set("pmTglAnalisa")} type="date" disabled={!canEdit(division, "pmTglAnalisa")} />
                </FieldWrapper>
                <FieldWrapper field="pmTglReport" division={division}>
                  <FInput label="Tgl Report" name="pmTglReport" value={form.pmTglReport}
                    onChange={set("pmTglReport")} type="date" disabled={!canEdit(division, "pmTglReport")} />
                </FieldWrapper>

                {form.pmTglAnalisa && (
                  <div className="col-span-2">
                    <LeadTimeBadge tglKirim={form.pmTglAnalisa} tglKeluar={form.pmTglReport} />
                  </div>
                )}

                <FieldWrapper field="pmHasilAnalisa" division={division}>
                  <FSelect label="Hasil Analisa" name="pmHasilAnalisa" value={form.pmHasilAnalisa}
                    onChange={set("pmHasilAnalisa")} options={STATUS_HASIL_OPTIONS} disabled={!canEdit(division, "pmHasilAnalisa")} />
                </FieldWrapper>
                <div className="col-span-2">
                  <FieldWrapper field="pmKeterangan" division={division}>
                    <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Keterangan</label>
                    <textarea name="pmKeterangan" value={form.pmKeterangan} rows={2}
                      onChange={e => set("pmKeterangan")(e.target.value)}
                      disabled={!canEdit(division, "pmKeterangan")}
                ...
                {!isNoLabscale(form.kodeItem) && (
                  <>
                    <SecHeader title="Alokasi Produk Labscale Kemasan Primer" badges={[
                      { text: "TS", color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" }
                    ]} />
                    <div className="col-span-2">
                      <ProductListEditor
                ...
                {isNoLabscale(form.kodeItem) && form.kodeItem.trim() !== "" && (
                  <>
                    <SecHeader title="Alokasi Produk Labscale Kemasan Primer" badges={[
                      { text: "TS", color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" }
                    ]} />
                    <div className="col-span-2">
                      <div className="px-4 py-3 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 text-xs text-amber-700 dark:text-amber-400 flex items-center gap-2">
                        <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        Kode item diawali <strong>{form.kodeItem.trim()[0]}</strong> — bagian Alokasi Produk Labscale Kemasan Primer tidak diperlukan.
                      </div>
                    </div>
                  </>
                )}

                <SecHeader title="Trial Mesin" badges={[
                  { text: "TS", color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" }
                ]} />

                <div className="col-span-2">
                  <FieldWrapper field="trialKodeProduk" division={division}>
                    <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Kode Produk</label>
                    <ProductAutocomplete
                      value={form.trialKodeProduk}
                      onChange={v => { set("trialKodeProduk")(v); setPendingTrialProduk(null) }}
                      disabled={!canEdit(division, "trialKodeProduk")}
                      refreshKey={produkRefreshKey}
                      onNewProductDetected={kode => setPendingTrialProduk(kode)}
                    />
                    {pendingTrialProduk && canEdit(division, "trialKodeProduk") && (
                      <div className="mt-2 px-3 py-2.5 bg-amber-50 dark:bg-amber-950/50 border border-amber-300 dark:border-amber-700 rounded-lg">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-2">
                            <svg className="w-4 h-4 text-amber-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <p className="text-xs text-amber-700 dark:text-amber-400">
                              Kode produk <strong>{pendingTrialProduk}</strong> belum ada di database.
                            </p>
                          </div>
                          <button type="button"
                            onClick={async () => { await saveNewProdukToDB(pendingTrialProduk!); setPendingTrialProduk(null) }}
                            className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold rounded-lg transition-colors whitespace-nowrap">
                            <SaveIcon /> Simpan Kode Produk
                          </button>
                        </div>
                ...
                <FieldWrapper field="trialNoBatch" division={division}>
                  <FInput label="No Batch" name="trialNoBatch" value={form.trialNoBatch}
                    onChange={set("trialNoBatch")} disabled={!canEdit(division, "trialNoBatch")} />
                </FieldWrapper>
                <FieldWrapper field="trialHasilFinal" division={division}>
                  <FSelect label="Hasil Final" name="trialHasilFinal" value={form.trialHasilFinal}
                    onChange={set("trialHasilFinal")} options={HASIL_OPTIONS} disabled={!canEdit(division, "trialHasilFinal")} />
                </FieldWrapper>
