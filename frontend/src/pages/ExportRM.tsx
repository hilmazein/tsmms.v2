...
import type { DiversifikasiRM } from "../types/types"
...
function KodeItemAutocomplete({ value, onChange, options }: {
  ...
  return (
    <div ref={wrapRef} className="relative">
      <input id="export-rm-kode-item" name="exportRmKodeItem" type="text" value={query}
...
interface ExportRMModalProps {
  data:    DiversifikasiRM[]
  onClose: () => void
}

export default function ExportRMModal({ data, onClose }: ExportRMModalProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/diversifikasi-rm/export?${params.toString()}`)
      if (!res.ok) {
      ...
      const fallback    = `Diversifikasi_RM_${dd}${mm}${yyyy}.xlsx`
      ...
      >
        <div className="flex items-center justify-between px-6 py-5 flex-shrink-0 rounded-t-2xl"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Export Data Diversifikasi RM</h3>
              <p className="text-xs text-white/60 mt-0.5">Format XLSX · Termasuk history revisi</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5 overflow-visible flex-1">
          ...
          <div
            ...
            style={{ maxHeight: mode === "custom" ? "500px" : "0px", opacity: mode === "custom" ? 1 : 0, pointerEvents: mode === "custom" ? "auto" : "none" }}
          >
            <div className="space-y-4 pt-1">
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">
                  Periode (berdasarkan Tgl Kirim CPro)
